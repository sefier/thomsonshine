...
function addFormEvent(formid, focus){
	...
	formNode[stmp[1]].onblur = function () {
		...
		checkpassword(formNode[stmp[1]].id, formNode[stmp[2]].id, formNode[stmp[0]].id);
	};
...
function checkpassword(id1, id2, id) {
	if(!$(id1).value && !$(id2).value) {
		return;
	}
	errormessage(id2);
	if($(id1).value.length < 5){
		errormessage(id1, '密码至少5位');
	}else if(mingxingtang_computeComplex($(id1).value, $(id).value) < 6){
		errormessage(id1, '密码过于简单，请尽量避免重复、有顺序规律或与用户名相似');
