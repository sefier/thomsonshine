 ...
 * @author xionghui
 * @author yaoying
 * @version $Id: xwb_install.class.php 591 2011-01-20 01:59:40Z yaoying $
 *
 */
class xwb_install {
	var $tpl_dir	= '';
	...
	function _dataInit(){
		...
		foreach($this->v['prepare_sql'] as $name => $format){
			$prepareSql = sprintf($format, XWB_S_TBPRE . $name);
			$st = $db->query($prepareSql);
		}
		
        $tmpRs = $db->query('SHOW COLUMNS FROM ' . XWB_S_TBPRE . 'xwb_bind_thread LIKE "type"');
        if ( !$db->num_rows($tmpRs))
        {
            $tbAlter = $this->v['alter_table'];
            foreach ($tbAlter as $name => $format)
            {
                $alterSql = sprintf($format, XWB_S_TBPRE . $name);
	...
	function _isPassEnvCheck(){
		if( $this->_sess->getInfo('check_succ_ck_name') != 1 ){
			$this->error('你还没有通过环境检查，请重新检查你的服务器环境！<br /><br />
							...
							此情况下插件将不能正常工作。<br />
							请检查php.ini中有关session的设置（<a href="http://bbs.x.weibo.com/forum/viewthread.php?tid=45" target="_blank">排错文档下载</a>），或者到论坛反馈。');
		}
	}
