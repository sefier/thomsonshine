 ...
 * $Id: xwbSiteUserRegister.class.php 497 2010-12-29 02:41:11Z yaoying $
 *
 */
class xwbSiteUserRegister{
	...
	function _updateSiteUserCount(){
		...
		//更新最新注册
		$totalmembers = $this->db->result_first("SELECT COUNT(*) FROM ". XWB_S_TBPRE. 'common_member');
	...
	function _updateRegCTR(){
		...
		if( $this->regfloodcount == 0 ){
			$this->db->query("INSERT INTO ". XWB_S_TBPRE. "common_regip (ip, count, dateline)
				VALUES ('{$this->ip}', '1', '{$this->timestamp}')");
		}elseif( $this->regfloodcount > 0 ){
			$this->db->query("UPDATE ". XWB_S_TBPRE. "common_regip SET count=count+1 WHERE ip='{$this->ip}' AND count>'0'");
		}
		if ($_G ['setting'] ['regctrl'] || $_G ['setting'] ['regfloodctrl']) {
			$this->db->query( "DELETE FROM ". XWB_S_TBPRE. "common_regip WHERE dateline<='{$this->timestamp}'-" . ($_G ['setting'] ['regctrl'] > 72 ? $_G ['setting'] ['regctrl'] : 72) . "*3600", 'UNBUFFERED' );
	...
	function _updateUserRegVerify(){
		...
		if( $_G['setting']['regverify'] == 2  || $_G['setting']['regverify'] == 3 ) {
			...
			$this->db->query("REPLACE INTO ". XWB_S_TBPRE. "common_member_validate (uid, submitdate, moddate, admin, submittimes, status, message, remark)
		...
		//EMAIL验证
			...
			$this->db->query("UPDATE ". XWB_S_TBPRE. "common_member_field_forum SET authstr='$authstr' WHERE uid='{$this->uid}'");
	...
	function _checkRegIP(){
		...
		if ($_G ['setting'] ['regctrl']) {
			$result = $this->db->result_first( "SELECT ip FROM " . XWB_S_TBPRE. "common_regip WHERE ip LIKE '$ctrlip' AND count='-1' AND dateline>{$this->timestamp}-'" . $_G ['setting'] ['regctrl'] . "'*3600 LIMIT 1" );
			if (!empty($result)) {
				return -1002;
			}
		}
		...
		if ($_G ['setting'] ['regfloodctrl']) {
			if ($regattempts = $this->db->result_first( "SELECT count FROM " . XWB_S_TBPRE. "common_regip WHERE ip='{$this->ip}' AND count>'0' AND dateline>'{{$this->timestamp}}'-86400" )) {
