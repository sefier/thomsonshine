 ...
 * @author yaoying
 * @since 2010-12-22
 * @version $Id: pushbackDispatcher.class.php 595 2011-01-20 12:39:32Z yaoying $
 *
 */
class pushbackDispatcher{
	...
	function prepare($registerShutdown = true){
		...
		if( $lockStatus < 0 ){
			$this->_log('预处理启动失败！原因：存在进程锁[-1]或者没到下一次更新时间[-2]。实际错误代码：'. $lockStatus);
			return false;
		}
	...
	function _readProcessData(){
		$query = $this->_db->query('SELECT * FROM '. XWB_S_TBPRE. "common_cache WHERE `cachekey` IN ('xwb_pushback_processlock', 'xwb_pushback_nexttime', 'xwb_pushback_updatesec')");
		$res = array();
	...
	function _updateProcessLock($force = false){
		if( true == $force ){
			$this->_db->query('REPLACE INTO '. XWB_S_TBPRE. "common_cache (`cachekey`, `cachevalue`, `dateline`) VALUES ('xwb_pushback_processlock', '{$this->_thistime}', '{$this->_thistime}')");
		}else{
			$this->_db->query('INSERT IGNORE INTO '. XWB_S_TBPRE. "common_cache (`cachekey`, `cachevalue`, `dateline`) VALUES ('xwb_pushback_processlock', '{$this->_thistime}', '{$this->_thistime}')");
		}
	}
	...
	function _updateNextProcess($nextUpdateSec = null){
		...
		$this->_db->query('REPLACE INTO '. XWB_S_TBPRE. "common_cache (`cachekey`, `cachevalue`, `dateline`) VALUES  
	...
	function _classifyComments(){
		...
		foreach ( $this->_mapper->midMapGet() as $type => $mids  ){
			foreach ( $mids as $mid => $tid ){
				...
				}elseif( isset($this->_comments[(float)$mid]) ){
					$comment[$type][(float)$mid] = $this->_comments[(float)$mid];
				}
			}
		}
