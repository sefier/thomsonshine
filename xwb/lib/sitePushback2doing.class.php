 ...
 * @author yaoying
 * @since 2010-12-22
 * @version $Id: sitePushback2doing.class.php 521 2011-01-04 02:50:07Z yaoying $
 *
 */
class sitePushback2doing{
	...
	function _createContent( $data ){
		...
		$content =  '<img src="' . $baseurl . XWB_P_DIR_NAME . '/images/bgimg/icon_logo.png" />'.
							$nickname .
							'(<a href="http://t.sina.com.cn/' . $data['uid'] . '" target="_blank">'.
	...
	function _replaceSinaUrlToHTML($content){
		...
		$replace = "<a href=\"http://sinaurl.cn/\${1}\" target=\"_blank\">http://sinaurl.cn/\${1}</a>";
	...
	function execInsert(){
		if( !empty($this->_docommentData) ){
			$this->_db->query("INSERT INTO ". XWB_S_TBPRE. "home_docomment 
		...
		foreach( $this->_doingCountData as $doid => $data ){
			$this->_db->query("UPDATE ". XWB_S_TBPRE. "home_doing
