 ...
 * @author yaoying
 * @since 2010-12-22
 * @version $Id: pushbackInterface.mod.php 591 2011-01-20 01:59:40Z yaoying $
 *
 */
class pushbackInterface{
	...
	function cfgPage(){
		...
		$db = XWB_plugin::getDB();
		$query = $db->query('SELECT `cachekey`,`cachevalue` FROM '. XWB_S_TBPRE. "common_cache WHERE `cachekey` IN ('xwb_pushback_nexttime', 'xwb_pushback_lasttime')");
		$res = array();
	...
	function _importUserFromUC($userInfo){
		...
		$db = XWB_plugin::getDB();
		$exist_uid = intval($db->result_first("SELECT uid FROM ". XWB_S_TBPRE. "common_member WHERE uid='{$userInfo[0]}' LIMIT 0,1 "));
		if($exist_uid > 0){
			return true;
		}
		...
		$db->query("INSERT IGNORE INTO ". XWB_S_TBPRE. "common_member (uid, username, password, adminid, groupid, email)
			VALUES ('{$uid}', '{$username}', '{$password}', '0', '10', '{$email}')");
		$db->query("INSERT IGNORE INTO ". XWB_S_TBPRE. "common_member_status (uid)
			VALUES ('$uid')");
		$db->query("INSERT IGNORE INTO ". XWB_S_TBPRE. "common_member_profile (uid)
			VALUES ('$uid')");
		$db->query("INSERT IGNORE INTO ". XWB_S_TBPRE. "common_member_field_forum (uid)
			VALUES ('$uid')");
		$db->query("INSERT IGNORE INTO ". XWB_S_TBPRE. "common_member_field_home (uid)
			VALUES ('$uid')");
		$db->query("INSERT IGNORE INTO ". XWB_S_TBPRE. "common_member_count (uid)
			VALUES ('$uid')");
