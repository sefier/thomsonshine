 ...
 * @since 2010-12-02
 * @version $Id: session_storage_db.class.php 356 2010-12-06 01:45:30Z yaoying $
 */
class session_storage_db{
	...
	function close(){
		...
		$hitNum = rand(1,100);
		if( $hitNum >= $extentBase - 7 && $hitNum <= $extentBase + 7 ){
			$this->gc();
		}
	...
	function read($id){
		...
		$result = $this->_db->result_first( 'SELECT `data` FROM '. XWB_S_TBPRE. $this->_sessionTable. 
												" WHERE `sessionid` = '{$id}' " 
											);
	...
	function write($id, $sess_data){
		...
		$this->_db->query('REPLACE INTO '. XWB_S_TBPRE. $this->_sessionTable. 
	...
	function destroy($id){
		$id = mysql_real_escape_string($id);
		$this->_db->query('DELETE FROM '. XWB_S_TBPRE. $this->_sessionTable. 
							" WHERE `sessionid` = '{$id}' "
							);
	}
