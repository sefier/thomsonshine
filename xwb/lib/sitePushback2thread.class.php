 ...
 * @author yaoying
 * @since 2010-12-22
 * @version $Id: sitePushback2thread.class.php 521 2011-01-04 02:50:07Z yaoying $
 *
 */
class sitePushback2thread{
	...
	function _createContent( $data ){
		...
		$content = $content. "\n\n". 
							...
							'[size=2][color=gray]'. 
							'[url=http://t.sina.com.cn/' . $data['uid'] . ']' . 
	...
	function _replaceSinaUrlToUBB($content){
		...
		$replace = "[url=http://sinaurl.cn/\${1}]http://sinaurl.cn/\${1}[/url]";
	...
	function execInsert(){
		...
		if ( false == $this->_checkPostPeriods() ){
			foreach ($this->_threadData as $tid => $tidInfo){
				$this->_db->query("UPDATE ". XWB_S_TBPRE. "forum_thread SET lastposter='{$tidInfo['lastposter']}', lastpost='{$tidInfo['lastpost']}', replies=replies+{$tidInfo['replies']} WHERE tid='{$tid}'", 'UNBUFFERED');
			}
		}
		...
		if( $this->_userConfig['uid'] > 0 && $this->_userReplyCount > 0  ){
			$this->_db->query("UPDATE ". XWB_S_TBPRE. "common_member_count SET posts=posts+{$this->_userReplyCount} WHERE uid='{$this->_userConfig['uid']}'", 'UNBUFFERED');
		}
		...
		foreach ( $this->_forumData as $fid => $fidInfo ){
			if( false == $this->_checkPostPeriods() ){
				$this->_db->query("UPDATE ". XWB_S_TBPRE. "forum_forum SET lastpost='{$fidInfo['lastpost']}', posts=posts+{$fidInfo['posts']}, todayposts=todayposts+{$fidInfo['posts']} WHERE fid='$fid'", 'UNBUFFERED');
			}else{
				$this->_db->query("UPDATE ". XWB_S_TBPRE. "forum_forum SET todayposts=todayposts+{$fidInfo['todayposts']} ,modworks='1' WHERE fid='$fid'", 'UNBUFFERED');
			}
