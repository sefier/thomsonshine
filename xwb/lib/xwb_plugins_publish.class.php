 ...
 * @author junxiong
 * @since 2010-6
 * @version $Id: xwb_plugins_publish.class.php 591 2011-01-20 01:59:40Z yaoying $
 *
 */
class xwb_plugins_publish{
	...
	function thread( $tid, $pid, $subject = '', $message = '' ) {
		...
		if (!empty($ret['id'])) {
			...
			$sess->appendStat('ryz', array( 'uid' => (int)XWB_plugin::getBindInfo("sina_uid"), 'mid' => $mid ));
			
			//插入“已同步到......”到指定pid中。
			if( XWB_plugin::pCfg('wb_addr_display') ){
				$redirectURL = XWB_API_URL. $ret['user']['id']. '/statuses/'. $mid;
				$baseurl = XWB_plugin::siteUrl();

				$insertSyncUBB = '[size=2][color=gray]'. ' [img]' . $baseurl . XWB_P_DIR_NAME . '/images/bgimg/icon_logo.png[/img] '.
				...
				$db->query( 'UPDATE '. XWB_S_TBPRE. $posttable .' SET `bbcodeoff` = 0, `message` = CONCAT(`message`, \''.$messageAppend . '\') WHERE `pid` = \'' . $pid .'\''  );
			}
		}
    ...
    function forShare($tid)
    {
		...
		if(XWB_plugin::pCfg('is_upload_image'))
        {
            ...
            $iconLogo = XWB_plugin::siteUrl() . XWB_P_DIR_NAME . '/images/bgimg/icon_logo.png';
	...
	function reply( $tid, $pid, $message = '' ) {
		...
		if ( isset($rs['id']) && !empty($rs['id']) ) {
			//发帖同步统计上报
			$sess = XWB_plugin::getUser();
			$sess->appendStat('ryz', array( 'uid' => (int)XWB_plugin::getBindInfo("sina_uid"), 'mid' => $rs['id'] ));
		}
	...
	function _getImage($pid, $msg, $num = 1) {
		...
		$query = $db->query("SELECT * FROM ". XWB_S_TBPRE. "forum_attachment WHERE pid='{$pid}'");
	...
	function getThreadUrl($tid){
		...
		if( 1 != $enableFromuid && $_G['setting']['rewritestatus'] && in_array('forum_viewthread', $_G['setting']['rewritestatus'])) {
			$siteUrl = str_replace(array('http://', 'https://', '//'), array('', '', '/'), XWB_plugin::siteUrl());
	...
	function blogSync( $id, $subject = '' ){
		...
		if (!empty($ret['id'])) {
			...
			if( XWB_plugin::pCfg('wb_addr_display') ){
				...
				$insertSyncHTML = "\n\r". '<DIV><FONT color="#808080" size=2><IMG src="'. XWB_plugin::siteUrl() . XWB_P_DIR_NAME . '/images/bgimg/icon_logo.png">&nbsp;'. XWB_plugin::L('xwb_topic_has_sycn_to'). '&nbsp;<a href="'. $redirectURL. '" target="_blank">'. $_G['username']. XWB_plugin::L('xwb_topic_has_sycn_to_new_end'). '</A></FONT></DIV>';
				$insertSyncHTML = mysql_real_escape_string( $insertSyncHTML );
				
				$db->query( 'UPDATE '. XWB_S_TBPRE. 'home_blogfield SET `message` = CONCAT(`message`, \''.$insertSyncHTML . '\') WHERE `blogid` = \'' . $id .'\''  );
	...
	function articleSync( $id, $subject ){
		...
		if (!empty($ret['id'])) {
			...
			if( XWB_plugin::pCfg('wb_addr_display') ){
				...
				$db->query( 'UPDATE '. XWB_S_TBPRE. 'portal_article_content SET `content` = CONCAT(`content`, \''.$insertSyncHTML . '\') WHERE `aid` = \'' . $id .'\''  );
	...
	function isSync($id, $type){
		...
		$sql = 'SELECT * FROM ' . XWB_S_TBPRE . 'xwb_bind_thread WHERE `tid`=' . (int)$id. ' AND `type`="' . $type . '" LIMIT 1 ';
	...
	function insertSyncId($id, $mid, $type){
		...
		$sql = "INSERT IGNORE INTO " . XWB_S_TBPRE . "xwb_bind_thread (`tid`, `mid`, `type`) VALUES('{$id}', '{$mid}', '{$type}')";
		$db->query($sql);
		if ($db->affected_rows()) {
			return true;
		}
