...
if($_G['forum_attachpids'] != '-1' && !$_G['gp_archiver']) {
    ...
    dbd_parseattach($_G['forum_attachpids'], $_G['forum_attachtags'], $postlist, $skipaids);
}
...
function viewthread_procpost($post, $lastvisit, $ordertype, $special = 0) {
    ...
    if($post['username']) {
        ...
        $post['avatar'] = get_user_avatar($post['authorid']);
...
function dbd_parseattach($attachpids, $attachtags, &$postlist, $skipaids = array()) {
    global $_G;
    ...
    while($attach = DB::fetch($query)) {
        ...
        $attach['attachicon'] = $_G['setting']['discuzurl'].preg_replace('/^.*? src="([^"]*?)" .*$/', '/$1', $attach['attachicon']);
        ...
        if(!empty($attachtags[$attach['pid']]) && is_array($attachtags[$attach['pid']]) && in_array($attach['aid'], $attachtags[$attach['pid']])) {
            ...
            $replaceattach[$attach['pid']][] = dbd_attachinpost($attach);
            $attached = 1;
        }

        if(!$attached) {
            if($attach['isimage']) {
                $return_data = dbd_attachlist($attach);
                if ($return_data['url'])
...
function dbd_attachlist($attach) {
    global $_G;
    ...
    if($attach['attachimg'] && $_G['setting']['showimages'])
    {
        if(!$attach['price'] || $attach['payed']) { 
            if($_G['setting']['thumbstatus'] && $attach['thumb']) {
                if($attach['refcheck'])
                    $return['thumbnail_url'] = $board_url."/forum.php?mod=attachment{$is_archive}&aid={$aidencode}";
                else
                    $return['thumbnail_url'] = $board_url."/{$attach[url]}{$attach[attachment]}.thumb.jpg";
            }
            
            if($attach['refcheck'])
                $return['url'] = $board_url."/forum.php?mod=attachment{$is_archive}&aid={$aidencode}&noupdate=yes&nothumb=yes";
            else
                $return['url'] = $board_url."/{$attach[url]}{$attach[attachment]}";
        }
    } else {
        if((!$attach['price'] || $attach['payed']) && (!$attach['readperm'] || $_G['group']['readaccess'] >= $attach['readperm'])) {
            if($attach['refcheck'])
                $return['url'] = $board_url."/forum.php?mod=attachment{$is_archive}&aid={$aidencode}&noupdate=yes";
            else
                $return['url'] = $board_url."/{$attach[url]}{$attach[attachment]}";
        }
    } 
...
function dbd_attachinpost($attach) {
    global $_G;
    ...
    if($attach['attachimg'] && $_G['setting']['showimages'] && (!$attach['price'] || $attach['payed'])) {
        if($attach['refcheck']) {
            $return .= "[img]$board_url/forum.php?mod=attachment{$is_archive}&aid={$aidencode}&noupdate=yes[/img]";
        } else {
            $return .= "[img]$board_url/$attach[url]$attach[attachment][/img]";
        }
        ...
        $return .= '[img]'.$attach['attachicon'].'[/img]';
        
        if($attach['description']) { 
            $return .= $attach['description'].'<br>';
        }
        
        $return .= $attach['filename']." ($attach[attachsize], 下载次数: $attach[downloads])";
