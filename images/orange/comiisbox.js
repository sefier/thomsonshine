	...
	return{
		...
		c=T$('comiis_show').innerHTML;
			...
			if(!a&&!u){
				...
				p.style.backgroundImage='none'; b.innerHTML=document.getElementById('comiis_show').innerHTML;
			}else{
...
for(var i = 0 ; i < tab.length ; i++){
	tab[i].onclick="open()";
	tab[i].href="javascript:open();";
	tab[i].target="_self";
}
var tabs = document.getElementsByTagName("form");
	for(var i = 0 ; i < tabs.length ; i++){
	tabs[i].action="javascript:;";
	tabs[i].target="_self";
}
function open(){TINY.box.show('',0,0,0,0)}
