...
class Ming_Password
{
	...
	public static function computeComplex($password, $username)
	{
		$complex = 0;
		$password = self::transfer((string)$password, $username);
		...
		for($i = 0; $i < $length; $i++){
			...
			if($prefixType != $type || !in_array($password[$i], $regularChars)){ //规律性字符，不计入复杂度
	...
	private static function transfer($password, $username)
	{
		if(($pos = strpos($password, $username)) !== false){
			return self::truncate($password, $username, $pos);
		}elseif(($pos = strpos($username, $password)) !== false){
			return self::truncate($username, $password, $pos);
		}
		
		return $password;
	}
	
	private static function truncate($long, $short, $pos)
	{
		return substr($long, 0, $pos).$short[0].substr($long, $pos + strlen($short));
	}
	...
	private static function getRegularChars($prefix, $prefixType)
	{
		...
		switch($prefixType){
			case 1:
				($prefix != 9) && ($regularChars[] = $prefix + 1);
				($prefix != 0) && $regularChars[] = $prefix - 1;
				break;
			...
			case 3:
				...
				$regularChars[] = chr(ord($prefix) - 1);
				$regularChars = array_filter($regularChars, 'ctype_alpha');
				break;
		}
