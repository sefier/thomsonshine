...
class logging_ctl {
	...
	function on_login() {
		...
		if(!submitcheck('loginsubmit', 1, $seccodestatus)) {
			...
			if(log_isban_mingxing($_SERVER['REMOTE_ADDR'])){
				showmessage('profile_ip_banned_mingxing');
			}
		...
		if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
			...
			if(!$activation) {
				...
				if(empty($this->setting['ignorepassword'])) {
					...
					if(strlen($_G['gp_password']) < 5){
						showmessage('profile_password_tooshort_tmx');
					}
					
					if(Ming_Password::computeComplex($_G['gp_password'], $username) < 6){
						showmessage('profile_password_register_toosimple_tmx');
					}
