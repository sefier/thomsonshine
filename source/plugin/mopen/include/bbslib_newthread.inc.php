...
require_once (MOPEN_ROOT.'./include/bbslib_post.inc.php');
...
if(!$_G['uid'] && !((!$_G['forum']['postperm'] && $_G['group']['allowpost']) || ($_G['forum']['postperm'] && forumperm($_G['forum']['postperm'])))) {
    mo_get_error('postperm_login_nopermission', array());
} elseif(empty($_G['forum']['allowpost'])) {
    if(!$_G['forum']['postperm'] && !$_G['group']['allowpost']) {
        mo_get_error('postperm_none_nopermission', array());
    } elseif($_G['forum']['postperm'] && !forumperm($_G['forum']['postperm'])) {
        mo_get_error('postperm', array());
    }
} elseif($_G['forum']['allowpost'] == -1) {
    mo_get_error('post_forum_newthread_nopermission', array());
}
...
if(!submitcheck('topicsubmit', 0, $seccodecheck, $secqaacheck)) {
    ...
    if($post_invalid = checkpost($subject, $message, ($special || $sortid))) {
        mo_get_error($post_invalid,  array('minpostsize' => $_G['setting']['minpostsize'], 'maxpostsize' => $_G['setting']['maxpostsize']));
    }

    if(checkflood()) {
        mo_get_error('post_flood_ctrl', array('floodctrl' => $_G['setting']['floodctrl']));
    } elseif(checkmaxpostsperhour()) {
        mo_get_error('post_flood_ctrl_posts_per_hour', array('posts_per_hour' => $_G['group']['maxpostsperhour']));
    }
    ...
    if($special == 1) {
        ...
        if(count($polloption) > $_G['setting']['maxpolloptions']) {
            mo_get_error('post_poll_option_toomany',  array('maxpolloptions' => $_G['setting']['maxpolloptions']));
        } elseif(count($polloption) < 2) {
            mo_get_error('post_poll_inputmore');
        }
        ...
        if(preg_match("/^\d*$/", trim($_G['gp_expiration']))) {
            ...
            mo_get_error('poll_maxchoices_expiration_invalid');
        }
        ...
        if($rewardprice < 1) {
            mo_get_error('reward_credits_please');
        } elseif($rewardprice > 32767) {
            mo_get_error('reward_credits_overflow');
            ...
            if($_G['group']['maxrewardprice'] > 0) {
                mo_get_error('reward_credits_between', array('minrewardprice' => $_G['group']['minrewardprice'], 'maxrewardprice' => $_G['group']['maxrewardprice']));
            } else {
                mo_get_error('reward_credits_lower',  array('minrewardprice' => $_G['group']['minrewardprice']));
            }
            ...
            mo_get_error('reward_credits_shortage');
        }
        $price = $rewardprice;
        ...
        if(count($extfield) > $_G['setting']['activityextnum']) {
            mo_get_error('post_activity_extfield_toomany',  array('maxextfield' => $_G['setting']['activityextnum']));
        }
        ...
        if(empty($_G['gp_affirmpoint']) || empty($_G['gp_negapoint'])) {
            ...
            if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_member')." WHERE username='$_G[gp_umpire]'")) {
                ...
                mo_get_error('debate_umpire_invalid',  array('umpire' => $umpire));
            }
        }
    ...
    if($_G['forum']['threadsorts']['expiration'][$typeid] && !$typeexpiration) {
        mo_get_error('threadtype_expiration_invalid');
    }
    ...
    if($modnewthreads) {
        ...
        //mo_get_error('post_newthread_mod_succeed',  $param);
    } else {
        ...
        //mo_get_error('post_newthread_succeed',  $param);

    }
}

$uid = $_G['uid'];
$post_succ = mo_get_message('define:post_succ');
...
$tpl->setVariable('post_succ', $post_succ);
//日志回传相关
