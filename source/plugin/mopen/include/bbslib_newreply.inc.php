...
if($_G['setting']['commentnumber'] && !empty($_G['gp_comment'])) {
    ...
    if($thread['closed'] && !$_G['forum']['ismoderator'] && !$thread['isgroup']) {
        mo_get_error('post_thread_closed');
    } elseif(!$thread['isgroup'] && $post_autoclose = checkautoclose($thread)) {
        mo_get_error($post_autoclose, array('autoclose' => $_G['forum']['autoclose']));
    } elseif(checkflood()) {
    ...
    mo_get_error('comment_add_succeed', array('tid' => $post['tid']) );
}
...
if(!submitcheck('replysubmit', 0, $seccodecheck, $secqaacheck)) {
    ...
    if($thread['special'] == 2 && ((!isset($_G['gp_addtrade']) || $thread['authorid'] != $_G['uid']) && !$tradenum = DB::result_first("SELECT count(*) FROM ".DB::table('forum_trade')." WHERE tid='$_G[tid]'"))) {
        mo_get_error('trade_newreply_nopermission', NULL);
    }
    ...
    if(trim($subject) == '' && trim($message) == '' && $thread['special'] != 2) {
        ...
        mo_get_error($post_invalid, array('minpostsize' => $_G['setting']['minpostsize']));
    } elseif(checkflood()) {
        mo_get_error('post_flood_ctrl',  array('floodctrl' => $_G['setting']['floodctrl']));
    } elseif(checkmaxpostsperhour()) {
        mo_get_error('post_flood_ctrl_posts_per_hour', array('posts_per_hour' => $_G['group']['maxpostsperhour']));
    ...
    if(!empty($_G['gp_trade']) && $thread['special'] == 2 && $_G['group']['allowposttrade']) {
        ...
        if(!trim($_G['gp_item_name'])) {
            mo_get_error('trade_please_name');
        } elseif($_G['group']['maxtradeprice'] && $item_price > 0 && ($_G['group']['mintradeprice'] > $item_price || $_G['group']['maxtradeprice'] < $item_price)) {
            mo_get_error('trade_price_between', array('mintradeprice' => $_G['group']['mintradeprice'], 'maxtradeprice' => $_G['group']['maxtradeprice']));
        } elseif($_G['group']['maxtradeprice'] && $item_credit > 0 && ($_G['group']['mintradeprice'] > $item_credit || $_G['group']['maxtradeprice'] < $item_credit)) {
            mo_get_error('trade_credit_between',  array('mintradeprice' => $_G['group']['mintradeprice'], 'maxtradeprice' => $_G['group']['maxtradeprice']));
        } elseif(!$_G['group']['maxtradeprice'] && $item_price > 0 && $_G['group']['mintradeprice'] > $item_price) {
            mo_get_error('trade_price_more_than', array('mintradeprice' => $_G['group']['mintradeprice']));
        } elseif(!$_G['group']['maxtradeprice'] && $item_credit > 0 && $_G['group']['mintradeprice'] > $item_credit) {
            mo_get_error('trade_credit_more_than', array('mintradeprice' => $_G['group']['mintradeprice']));
        } elseif($item_price <= 0 && $item_credit <= 0) {
            mo_get_error('trade_pricecredit_need');
        } elseif($_G['gp_item_number'] < 1) {
            mo_get_error('tread_please_number');
        }
    ...
    if($modnewreplies) {
        ...
        //mo_get_error('post_reply_mod_succeed', $param);
    } else {
        ...
        //mo_get_error($replymessage, $param);
    }
