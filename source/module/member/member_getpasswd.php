...
if($_G['gp_uid'] && $_G['gp_id']) {
	...
	if(!submitcheck('getpwsubmit') || $_G['gp_newpasswd1'] != $_G['gp_newpasswd2']) {
		...
		if(strlen($_G['gp_newpasswd1']) < 5){
			showmessage('profile_passwd_short_tmx');
		}
		if(Ming_Password::computeComplex($_G['gp_newpasswd1'], $member['username']) < 6){
			showmessage('profile_passwd_simple_tmx');
		}
		...
		//密码增强
		$stamp = time();
		DB::query("UPDATE simple_password_recover SET enhanced=$stamp WHERE uid='$_G[gp_uid]'");
		
		//记录修改
		//对修改操作进行记录，防止恶意修改
		$ip_tmx = $_SERVER['REMOTE_ADDR'];
		DB::query("INSERT INTO simple_password_change(uid, old_password, new_password, old_email, new_email, changed, ip) VALUES('$_G[gp_uid]', '', '$_G[gp_newpasswd1]', '', '', $stamp, '$ip_tmx')");
